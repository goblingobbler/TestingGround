...
//This is a setIntervalLoop
( function () {

    var lastTime = 0;
    var vendors = [ 'ms', 'moz', 'webkit', 'o' ];

    //Search for the appropriate API function depending on the browser
    for ( var x = 0; x < vendors.length && !window.requestAnimationFrame; ++ x ) {
        window.requestAnimationFrame = window[ vendors[ x ] + 'RequestAnimationFrame' ];
        window.cancelAnimationFrame = window[ vendors[ x ] + 'CancelAnimationFrame' ] || window[ vendors[ x ] + 'CancelRequestAnimationFrame' ];
    }
    //setTimeout fallback if it couldn't be found
    if ( !window.requestAnimationFrame ) {
        window.requestAnimationFrame = function ( callback, element ) {
            var currTime = Date.now()
            //Set next interval with 0 fallback if lastTime hasn't been set
...
game_core.prototype.check_space_overlap = function(item){

	var x = Math.floor((item.pos.x - this.offset)/this.increment);
	var y = Math.floor((item.pos.y)/this.increment);
	if (x >= 0 && y >= 0 && x < this.board_length && y < this.board_length){
		if (this.board_spaces[x][y].state == 0){
			this.board_spaces[x][y].state = 1;
			if (this.board_spaces[x][y].bomb){
				item.score -= 20;
			} else {
				item.score += this.board_spaces[x][y].score;
			}
		}
	}

}; //game_core.check_space_overlap
...
game_core.prototype.server_update_physics = function() {
	...
	this.check_space_overlap(this.players.self);
	this.check_space_overlap(this.players.other);
...
game_core.prototype.server_update = function(){
    ...
    this.laststate = {
        board_spaces : this.board_spaces,
        self_score : this.players.self.score,
        other_score : this.players.other.score,
...
game_core.prototype.client_onconnected = function(data) {
    ...
    chatMessage( 'Connected successfully to the socket.io server. My server side ID is ' + data.id );
...
game_core.prototype.client_draw_board = function() {

        //We don't want this to be too distracting
    this.ctx.fillStyle = 'rgba(255,255,255,0.3)';
	var x = Math.floor((this.players.self.pos.x - this.offset)/this.increment);
	var y = Math.floor((this.players.self.pos.y)/this.increment);

	this.ctx.fillText(x + ' ' + y,10,30);
	this.ctx.fillText(this.players.self.score + ' ' + this.players.other.score,10,60);
	...
	for (var i=0;i<this.board_length;i++){
		for (var j=0;j<this.board_length;j++){
			...
			this.ctx.rect(i*this.increment + this.offset, j*this.increment, this.increment, this.increment);
			this.ctx.stroke();

			if (this.board_spaces[i][j].state == 0){
				this.ctx.fillStyle = 'rgba(255,255,255,0.4)';
				this.ctx.fillRect(i*this.increment + this.offset + 4, j*this.increment + 4, this.increment - 8, this.increment - 8);
			}
			else {
				this.ctx.font= (this.size-4) + "px Verdana";
				var center_offset = (this.increment - this.ctx.measureText('0').width)/2;
				if (this.board_spaces[i][j].bomb){
					this.ctx.fillStyle = 'rgba(255,50,50,.8)';
					this.ctx.fillText("X",i*this.increment + this.offset + center_offset, j*this.increment + this.increment - 6);
				} else {
					var hex = parseInt(255-(160*(this.board_spaces[i][j].score/8)));
					this.ctx.fillStyle = 'rgba(255,'+hex+','+hex+',.8)';
					this.ctx.fillText(this.board_spaces[i][j].score, i*this.increment + this.offset + center_offset, j*this.increment + this.increment - 6);
				}
			}
		}
	}
